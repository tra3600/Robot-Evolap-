...
def recherche_points(imgI, imgJ, pts):
    """
    Recherche les pixels déplacés dans l'image courante imgJ à partir des points d'intérêt dans imgI.
    
    Paramètres:
    imgI (2D array): Image précédente en niveaux de gris.
    imgJ (2D array): Image courante en niveaux de gris.
    pts (list): Liste des points d'intérêt de l'image précédente.
    
    Retourne:
    list: Liste des points déplacés dans l'image courante.
    """
    fpts = []
    
    for pt in pts:
        patch = creation_patch(pt, 5)
        A, b = calc_Ab(imgI, imgJ, patch)
        dx, dy = resoud_LK(A, b)
        new_x = int(pt[0] + dx)
        new_y = int(pt[1] + dy)
        fpts.append((new_x, new_y))
...
imgI = np.random.randint(0, 256, (800, 600))
imgJ = np.random.randint(0, 256, (800, 600))
pts = construction_coordonnees_pts([100, 150, 200, 100], 4, 3)
fpts = recherche_points(imgI, imgJ, pts)
print(fpts)



def verification_retour(imgI, imgJ, pts):
    """
    Vérifie si les points initiaux de imgI peuvent être retrouvés à partir des points obtenus dans imgJ.
    ...
    Retourne:
    list: Tableau de booléens indiquant si chaque point a été retrouvé.
    """
    fpts = recherche_points(imgI, imgJ, pts)
    statut = []
    
    for initial_pt, current_pt in zip(pts, fpts):
        re_found = recherche_points(imgJ, imgI, [current_pt])[0]
        if re_found == initial_pt:
            statut.append(True)
        else:
            statut.append(False)
...
# Exemple d'utilisation
statut = verification_retour(imgI, imgJ, pts)
print(statut)


def calcul1(points1, points2):
    """
    Calcule la distance euclidienne entre les points d'intérêt de l'image précédente et ceux de l'image courante.
    
    Paramètres:
    points1 (list): Liste des points d'intérêt de l'image précédente.
    points2 (list): Liste des points d'intérêt de l'image courante.
    
    Retourne:
    list: Distances euclidiennes entre les points correspondants.
    """
    return np.sqrt((np.array(points1[0::2]) - np.array(points2[0::2]))**2 + (np.array(points1[1::2]) - np.array(points2[1::2]))**2)



def mediane(a):
    """
    Calcule la valeur médiane d'un tableau de valeurs.
    
    Paramètres:
    a (list): Tableau de valeurs strictement positives de longueur impaire.
    
    Retourne:
    float: Valeur médiane du tableau.
    """
    a_sorted = sorted(a)
    n = len(a_sorted)
    return a_sorted[n // 2]
...
# Exemple d'utilisation
a = [3, 1, 4, 1, 5, 9, 2]
print(mediane(a))  # Devrait afficher 3


def verification_pts(pts, fpts, statut):
    """
    Vérifie les points obtenus et renvoie un nouveau tableau de points valides.
    
    Paramètres:
    pts (list): Liste des points d'intérêt de l'image précédente.
    fpts (list): Liste des points d'intérêt de l'image courante.
    statut (list): Liste de booléens indiquant la validité de chaque point.
    
    Retourne:
    list: Nouveau tableau de points valides.
    """
    valid_pts = [fpts[i] for i in range(len(statut)) if statut[i]]
    distances = calcul1(pts, valid_pts)
    med = mediane(distances)
    
    nouveaux_pts = [valid_pts[i] for i in range(len(distances)) if distances[i] <= med]
    return nouveaux_pts

# Exemple d'utilisation
nouveaux_pts = verification_pts(pts, fpts, statut)
print(nouveaux_pts)
...
def calculer_correlation(image1, image2, point, patch_size):
    """
    Calcule la corrélation croisée normée pour un point donné entre deux images.
    
    Paramètres:
    image1 (2D array): Première image.
    image2 (2D array): Deuxième image.
    point (tuple): Coordonnées du point (x, y).
    patch_size (int): Taille du patch.
    
    Retourne:
    float: Valeur de la corrélation croisée normée.
    """
    x, y = point
    half_size = patch_size // 2
    
    patch1 = image1[y-half_size:y+half_size+1, x-half_size:x+half_size+1]
    patch2 = image2[y-half_size:y+half_size+1, x-half_size:x+half_size+1]
    
    result = cv2.matchTemplate(patch2, patch1, cv2.TM_CCORR_NORMED)
    return result[0][0]

# Exemple d'utilisation
correlation = calculer_correlation(imgI, imgJ, (100, 100), 5)
print(correlation)